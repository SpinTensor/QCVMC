...
#include <spherical_harmonics_l4_m0.h>

double reference_spherical_harmonics_l4_m0(double x, double y, double z) {
   double r = sqrt(x*x + y*y + z*z);
   return (3.0/16.0) * sqrt(1.0/M_PI) * (35.0*z*z*z*z - 30.0*z*z*r*r + 3.0*r*r*r*r) / pow(r, 4);
}
   ...
   for (double r=rmin; r<=rmax; r+=deltar) {
      for (double phi=phimin; phi<=phimax; phi+=deltaphi) {
         for (double theta=thetamin; theta<=thetamax; theta+=deltatheta) {
            ...
            double reference = reference_spherical_harmonics_l4_m0(x, y, z);
            double testval = spherical_harmonics_l4_m0(x, y, z);
