...
#include <spherical_harmonics_l3_mm2.h>

double reference_spherical_harmonics_l3_mm2(double x, double y, double z) {
   double r = sqrt(x*x + y*y + z*z);
   return 0.5*sqrt(105.0/M_PI) * x * y * z / pow(r, 3);
}
   ...
   for (double r=rmin; r<=rmax; r+=deltar) {
      for (double phi=phimin; phi<=phimax; phi+=deltaphi) {
         for (double theta=thetamin; theta<=thetamax; theta+=deltatheta) {
            ...
            double reference = reference_spherical_harmonics_l3_mm2(x, y, z);
            double testval = spherical_harmonics_l3_mm2(x, y, z);
