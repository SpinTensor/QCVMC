...
#include <spherical_harmonics_l2_m0.h>

double reference_spherical_harmonics_l2_m0(double x, double y, double z) {
   double r2 = x*x + y*y + z*z;
   return 0.25*sqrt(5.0/M_PI) * (2.0*z*z - x*x - y*y) / r2;
}
   ...
   for (double r=rmin; r<=rmax; r+=deltar) {
      for (double phi=phimin; phi<=phimax; phi+=deltaphi) {
         for (double theta=thetamin; theta<=thetamax; theta+=deltatheta) {
            ...
            double reference = reference_spherical_harmonics_l2_m0(x, y, z);
            double testval = spherical_harmonics_l2_m0(x, y, z);
