...
#include <spherical_harmonics_l2_mp1.h>

double reference_spherical_harmonics_l2_mp1(double x, double y, double z) {
   double r2 = x*x + y*y + z*z;
   return 0.5*sqrt(15.0/M_PI) * x * z / r2;
}
   ...
   for (double r=rmin; r<=rmax; r+=deltar) {
      for (double phi=phimin; phi<=phimax; phi+=deltaphi) {
         for (double theta=thetamin; theta<=thetamax; theta+=deltatheta) {
            ...
            double reference = reference_spherical_harmonics_l2_mp1(x, y, z);
            double testval = spherical_harmonics_l2_mp1(x, y, z);
