...
#include <spherical_harmonics_l4_mp4.h>

double reference_spherical_harmonics_l4_mp4(double x, double y, double z) {
   double r = sqrt(x*x + y*y + z*z);
   return (3.0/16.0) * sqrt(35.0/M_PI) * (x*x*(x*x-3.0*y*y) - y*y*(3.0*x*x - y*y)) / pow(r, 4);
}
   ...
   for (double r=rmin; r<=rmax; r+=deltar) {
      for (double phi=phimin; phi<=phimax; phi+=deltaphi) {
         for (double theta=thetamin; theta<=thetamax; theta+=deltatheta) {
            ...
            double reference = reference_spherical_harmonics_l4_mp4(x, y, z);
            double testval = spherical_harmonics_l4_mp4(x, y, z);
